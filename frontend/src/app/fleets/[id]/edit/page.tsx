...
export default function EditFleetPage() {
  ...
  useEffect(() => {
    if (data?.data && !populated) {
      ...
      setForm({
        ...
        region: fleet.region ?? '',
        monthlyBudget: fleet.monthlyBudget ? String(parseFloat(fleet.monthlyBudget)) : '',
  ...
  const mutation = useMutation({
    mutationFn: (payload: UpdatePayload) =>
      api.patch<{ success: boolean; data: FleetDetail }>(`/fleets/${id}`, payload),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['fleets'] });
      qc.invalidateQueries({ queryKey: ['fleet', id] });
      router.push(`/fleets/${id}`);
    },
  });
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <Link href={`/fleets/${id}`} className="text-sm text-gray-500 hover:text-gray-700">{fleet.name}</Link>
          <span className="text-gray-300">/</span>
          <span className="text-sm font-medium text-gray-900">Edit</span>
        </div>

        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          <h1 className="mb-6 text-lg font-semibold text-gray-900">Edit Fleet</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {mutation.isError && (
              <div className="rounded border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                {mutation.error instanceof Error ? mutation.error.message : 'Failed to update fleet. Please try again.'}
              </div>
            )}

            <div className="flex justify-end gap-3 pt-2">
              <Link href={`/fleets/${id}`} className="rounded-md px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100">Cancel</Link>
